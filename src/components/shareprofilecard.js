  ...
  return (
    <div className="shareProfileCardPannel">
      <button type="button" title="Copy" onClick={handleCopyLink}>
        <div>
          {copy ? (
            <TiTick className="shareProfileCardTickIcon" />
          ) : (
            <MdContentCopy />
          )}
        </div>
      ...
      <a
        target="_blank"
        rel="noreferrer"
        href={`https://api.whatsapp.com/send?text=${"Hey, Check out my dog's account on Wiggles: "} ${url}`}
      >
        <div>
          <BsWhatsapp />
        </div>
      ...
      <a
        target="_blank"
        rel="noreferrer"
        href={`https://twitter.com/share?url=${url}&text=[post-title]&via=[via]&hashtags=[hashtags]`}
      >
        <div>
          <BsTwitter />
        </div>
